...
const KEY = "oasis_diagnostics_suite_tabs_v1";
...
function updateKPIs(){
  ...
  kpiClient.textContent = (client.value||"").trim() ? (client.value||"").trim() : "—";
...
function saveDiag(){
  ...
  if (idx >= 0) db.history[idx] = { ...db.history[idx], ...p };
  else db.history.unshift({ ...p, createdAt: new Date().toISOString() });
...
function openDiag(id){
  ...
  updateKPIs();
  setView("diag"); // te lleva a la pestaña Diagnóstico
...
function renderCodeResults(rows){
  ...
  codeResults.querySelectorAll("[data-use]").forEach(b=>{
    b.addEventListener("click", ()=>{
      ...
      updateKPIs();
      setView("diag"); // vuelve a Diagnóstico
      diagnosis.focus();
    });
  });
